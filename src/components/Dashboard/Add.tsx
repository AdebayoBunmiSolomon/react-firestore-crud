...
export const Add = ({ setIsAdding }: IAddProps) => {
  ...
  const { useAddEmployee, loading, imageUrl } = AddEmployeeService();

  const handleAdd = async (data: addEmployee) => {
    // const imgUrl: string = await useGetImageUrl(imgUpload);
    // data.imageUpload = imageUrl;
  ...
  return (
    <div className='flex justify-center items-center h-screen mt-10'>
      {/* <img src={imageUrl && imageUrl} className='w-10 h-10 self-center' /> */}
